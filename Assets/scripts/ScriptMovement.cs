...
{
    /// <summary>
    /// Объявлям переменную связывающую другой скрипт
    /// </summary>
  ...
  //  private static Transform[,] grid = new Transform[width, height];
    ...
    void Start()
    {
        ...
        speed = PlayerPrefs.GetInt("SpeedSelection", 1);//Узнаем какая  скорость была выбрана
    ...
    /// <summary>
    /// Общее движение для двух режимов
    /// </summary>
    ...
    private void CheckForLinesMode2()
    {
        ...
        for (int i = height - 1; i >= 0; i -=2)
        {
            ...
            if (HasLine(i) && HasLine(i - 1))//Если такая линия найдена удаляем ее и смещаемся вниз
            {
                ...
                scriptText.AddScore((i + 2) * (i + 2));//В зависимости от того на какой строке была собрана строка столько очков и будет добавлено
            }
        }
    }
